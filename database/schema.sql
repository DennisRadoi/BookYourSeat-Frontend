-- Baza de date pentru aplicatia de rezervare locuri la birou.
-- Schema porneste de la diagrama existenta: User, Departament, Adresa,
-- Localitate, Judet, Cladire, Sala, Loc.
...
CREATE TABLE rezervare_loc (
  ...
  CONSTRAINT fk_rezervare_loc_utilizator
    FOREIGN KEY (utilizator_id) REFERENCES utilizator(id),
  CONSTRAINT fk_rezervare_loc_loc
    FOREIGN KEY (loc_id) REFERENCES loc(id),
  CONSTRAINT chk_rezervare_loc_interval
    CHECK (ora_start < ora_final),
  CONSTRAINT chk_rezervare_loc_status
...
CREATE TABLE preferinta_utilizator (
  ...
  CONSTRAINT fk_preferinta_utilizator
    FOREIGN KEY (utilizator_id) REFERENCES utilizator(id),
  CONSTRAINT chk_preferinta_utilizator_interval
    CHECK (
...
CREATE TABLE invitatie_birou (
  ...
  CONSTRAINT fk_invitatie_rezervare
    FOREIGN KEY (rezervare_loc_id) REFERENCES rezervare_loc(id),
...
CREATE TABLE notificare (
  ...
  CONSTRAINT fk_notificare_destinatar
    FOREIGN KEY (destinatar_id) REFERENCES utilizator(id),
  CONSTRAINT fk_notificare_rezervare_loc
    FOREIGN KEY (rezervare_loc_id) REFERENCES rezervare_loc(id),
...
CREATE TABLE insight_ai (
  ...
  utilizator_id INT NOT NULL,
  rezervare_loc_id BIGINT,
  raport_vreme_id BIGINT,
  raport_trafic_id BIGINT,
...
CREATE INDEX idx_loc_sala_status ON loc(id_sala, status);
CREATE INDEX idx_rezervare_loc_data ON rezervare_loc(data_rezervare);
CREATE INDEX idx_rezervare_loc_utilizator_data ON rezervare_loc(utilizator_id, data_rezervare);
CREATE INDEX idx_rezervare_loc_loc_data ON rezervare_loc(loc_id, data_rezervare);
CREATE INDEX idx_notificare_destinatar ON notificare(destinatar_id, citita_la);
CREATE INDEX idx_raport_vreme_cladire_moment ON raport_vreme(cladire_id, moment_raport);
CREATE INDEX idx_raport_trafic_cladire_moment ON raport_trafic(cladire_id, moment_raport);
