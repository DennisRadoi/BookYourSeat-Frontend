...
function StatusBadge({ status }: { status: Colleague["status"] }) { return <span className={`inline-flex min-w-[74px] justify-center rounded-full px-2.5 py-1 text-[11px] font-bold ${status === "La birou" ? "bg-[var(--secondary)] text-[var(--secondary-foreground)]" : "bg-[var(--destructive)]/20 text-[var(--destructive)]"}`}>{status}</span> }
