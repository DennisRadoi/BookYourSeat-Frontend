...
export default function SetariPage() {
  ...
  return (
    <section className="max-w-[960px] text-[#27312f]">
      <div className="mb-4 flex items-start justify-between gap-4 border-b border-[#e5e7e8] pb-3">
        <div>
        ...
        <button
          ...
          className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-[#08a477] px-4 text-xs font-bold text-white shadow-sm transition hover:bg-[#078b65] focus:outline-none focus:ring-2 focus:ring-[#08a477] focus:ring-offset-2"
        >
          {saved && <Check size={15} />}
      ...
      <div className="grid max-w-[760px] gap-6 lg:grid-cols-2">
        <div className="space-y-6">
          <fieldset>
            <legend className="mb-2 text-[10px] font-bold uppercase tracking-wide text-[#8a9291]">Preferințe muncă</legend>
            <div className="rounded-xl bg-white p-4 shadow-[0_1px_4px_rgba(0,0,0,0.07)]">
        ...
        <fieldset>
          ...
          <div className="divide-y divide-[#eef0f1] rounded-xl bg-white px-4 shadow-[0_1px_4px_rgba(0,0,0,0.07)]">
...
function ToggleRow({ title, description, enabled, onChange }: { title: string; description: string; enabled: boolean; onChange: (value: boolean) => void }) {
  return <div className="flex min-h-[61px] items-center justify-between gap-3 py-3"><div><p className="text-xs font-semibold text-[#4c5653]">{title}</p><p className="mt-0.5 text-[10px] text-[#929997]">{description}</p></div><button type="button" role="switch" aria-checked={enabled} onClick={() => onChange(!enabled)} className={`relative h-4 w-7 shrink-0 rounded-full transition ${enabled ? "bg-[#08a477]" : "bg-[#d9dcdf]"}`}><span className={`absolute top-0.5 size-3 rounded-full bg-white shadow transition ${enabled ? "left-3.5" : "left-0.5"}`} /></button></div>
}
